...
import kotlinx.android.synthetic.main.activity_upload_location.*

class UploadLocationActivity : BaseActivity() {
    private lateinit var listener: MyLocationListener
    override fun getLayoutId(): Int = R.layout.activity_upload_location
    ...
    override fun initViews() {
        ...
        option.priority = LocationClientOption.GpsFirst
                ...
                if (contentEdit.text.isNotEmpty()) {
                    StudentApi.uploadLocation(address, contentEdit.text.toString(), longitude, latitude, object : RequestCallBack<String>() {
                        ...
                        override fun onSuccess(entity: String?) {
                            T.show(this@UploadLocationActivity, "上传成功")
                            setResult(1000)
                            finish()
                        }

                        override fun onError(exception: Throwable) {
                            exception.handleThrowable(this@UploadLocationActivity)
                        }

                    })
                } else {
                    T.show(this@UploadLocationActivity, "请输入备注")
                }
            } else {
                T.show(this@UploadLocationActivity, "尚未获取到位置信息")
            }
