...
//            .setOutputCameraPath("/CustomPath")// 自定义拍照保存路径,可不填
            .enableCrop(cropEnable)// 是否裁剪 true or false
...
//            .freeStyleCropEnabled()// 裁剪框是否可拖拽 true or false
            .circleDimmedLayer(circleCrop)// 是否圆形裁剪 true or false
            .showCropFrame(circleCrop)// 是否显示裁剪矩形边框 圆形裁剪时建议设为false   true or false
            .showCropGrid(circleCrop)// 是否显示裁剪矩形网格 圆形裁剪时建议设为false    true or false
...
fun Fragment.startSelectPicture(isMultiple: Boolean){
    this.startSelectPicture(false, false, 8, isMultiple)
}
fun Fragment.startSelectPicture(cropEnable: Boolean = false, circleCrop: Boolean = false, maxSelectNum: Int = 8, isMultiple: Boolean = true){
    PictureSelector.create(this)
            .openGallery(PictureMimeType.ofImage())//全部.PictureMimeType.ofAll()、图片.ofImage()、视频.ofVideo()、音频.ofAudio()
//            .theme(R.style.picture.default.style)//主题样式(不设置为默认样式) 也可参考demo values/styles下 例如：R.style.picture.white.style
            .maxSelectNum(maxSelectNum)// 最大图片选择数量 int
