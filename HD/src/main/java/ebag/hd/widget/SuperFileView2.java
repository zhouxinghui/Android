...
public class SuperFileView2 extends FrameLayout implements TbsReaderView.ReaderCallback {
    ...
    public void displayFile(File mFile) {
        ...
        if (mFile != null && !TextUtils.isEmpty(mFile.toString())) {
            ...
            if (!bsReaderTempFile.exists()) {
                L.INSTANCE.d("准备创建/storage/emulated/0/TbsReaderTemp！！");
                boolean mkdir = bsReaderTempFile.mkdir();
                if(!mkdir){
                    L.INSTANCE.e("创建/storage/emulated/0/TbsReaderTemp失败！！！！！");
                }
            }

            //加载文件
            Bundle localBundle = new Bundle();
            L.INSTANCE.d(mFile.toString());
    ...
    private String getFileType(String paramString) {
        ...
        if (TextUtils.isEmpty(paramString)) {
            L.INSTANCE.d(TAG, "paramString---->null");
            return str;
        }
        L.INSTANCE.d(TAG, "paramString:" + paramString);
        int i = paramString.lastIndexOf('.');
        if (i <= -1) {
            L.INSTANCE.d(TAG, "i <= -1");
            return str;
        }


        str = paramString.substring(i + 1);
        L.INSTANCE.d(TAG, "paramString.substring(i + 1)------>" + str);
        return str;
    }
