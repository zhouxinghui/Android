...
import ebag.mobile.bean.CitysBean
...
class EditAddressActivity : BaseActivity() {
    ...
    private fun checkEmpty(name: EditText, phone: EditText, preAddress: TextView, address: EditText): Boolean {
        when {
            ...
            (!Pattern.matches(Constants.MOBILENUMBER_REGEX, phone.text.toString().trim()) && phone.text.toString().trim().length == 11) -> T.show(this, "手机号格式不正确")
            else -> return true
        }
