...
public class RfbProto implements RfbConnectable {
  ...
  //
  public RfbProto(Decoder decoder, RemoteCanvas canvas, String host, int port, int preferredEncoding,
           boolean viewOnly, boolean useLocalCursor,int timeOut) throws Exception {
      if(preferredEncoding == -1)  preferredEncoding = 7;
      setParameters(decoder, canvas, host, port, preferredEncoding, viewOnly, useLocalCursor, timeOut);
  }
  ...
  public void initializeAndAuthenticate(String us, String pw) throws Exception {
		...
		int secType = negotiateSecurity(bitPref);
		int authType;
  ...
  public int negotiateSecurity(int bitPref) throws Exception {
      if (clientMinor >= 7) {
          return selectSecurityType(bitPref);
      } else {
  ...
  */
  ...
  public void clientRedirect(int port, String host) {
      try {
          closeSocket();
          setParameters(decoder, canvas, host, port, preferredEncoding, viewOnly, useLocalCursor,Constants.SOCKET_CONN_TIMEOUT);
          writeClientInit();
          readServerInit();
          processProtocol();
      } catch (Exception e) {
