...
abstract class BaseLoginSelectActivity : BaseActivity(), View.OnClickListener {
    companion object {
    ...
    override fun onClick(p0: View) {
        when (p0.id) {
            R.id.btn_binding -> {
                startActivity(Intent(this, getBindingClass())
                        .putExtra("BX", "b")
                        .putExtra("name", name)
    ...
    fun cancelThreeParty(share_media: SHARE_MEDIA) {
        UMShareAPI.get(this).deleteOauth(this, share_media, object : UMAuthListener {
            override fun onComplete(p0: SHARE_MEDIA?, p1: Int, p2: MutableMap<String, String>?) {
                Log.d("取消授权成功")
                Toast.makeText(this@BaseLoginSelectActivity, "取消授权成功", Toast.LENGTH_SHORT)
            }
    ...
    abstract fun getBindingClass(): Class<*>?
}
