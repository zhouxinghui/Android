...
import ebag.mobile.module.clazz.album.AlbumDetailActivity.Adapter.Companion.TYPE_STICKY_HEAD
import ebag.mobile.widget.stickyItem.FullSpanUtil
import ebag.mobile.widget.stickyItem.StickyHeadContainer
import ebag.mobile.widget.stickyItem.StickyItemDecoration
...
//            showOptions(true)
            modifyPopup.showAsDropDown(uploadBtn)
            isAllChoose = false
    ...
    override fun requestData(page: Int, requestCallBack: RequestCallBack<ArrayList<PhotoBean>>) {
        EBagApi.albumDetail(photoGroupId, groupType, page, 50, requestCallBack)
    }
