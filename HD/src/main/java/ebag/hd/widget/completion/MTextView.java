...
public class MTextView extends TextView {
    ...
    SparseArray<ISAttributes> isAttributes = new SparseArray<>();
//	private float lineSpacingMult = 0.5f;
    ...
    public MTextView(Context context) {
        ...
        lineSpacing = dip2px(lineSpacingDP);
        minHeight = dip2px(30);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < contentList.size(); i++) {
            ...
            for (int j = 0; j < line.line.size(); j++) {
                ...
                } else if (ob instanceof TypeImageSpan) {
                    TypeImageSpan is = (TypeImageSpan) ob;
                    Drawable d = is.getDrawable();

                    int left = (int) (realDrawedWidth)+dip2px(2);
                    int top = (int) height;
                    int right = (int) (realDrawedWidth + width) -dip2px(2);
                    int bottom = (int) (height + line.height);
                    d.setBounds(left, top, right, bottom);
                    d.draw(canvas);
                    realDrawedWidth += width;
                    ISAttributes attributes = new ISAttributes();
                    attributes.setLeft(left);
                    attributes.setRight(right);
                    attributes.setTop(top);
                    attributes.setBottom(bottom);
                    attributes.setType(is.getType());
                    isAttributes.append(index,attributes);
                    index++;
                }
    ...
    public SparseArray<ISAttributes> getListMap() {
    ...
    private int measureContentHeight(int width) {
        ...
        for (int i = 0; i < obList.size(); i++) {
            ...
            if (ob instanceof String) {
                ...
                tIS = (TypeImageSpan) ob;
                if("#F#".equals(tIS.getType())){
                    obWidth = width - drawedWidth;
                    obHeight = 1;
                }else{
                    Rect r = ((TypeImageSpan) ob).getDrawable().getBounds();
                    obWidth = spaces.get(++ss);
    ...
    public void setMText(CharSequence cs) {
        ...
        ArrayList<IS> isList = new ArrayList<>();
        useDefault = false;
