...
import ebag.hd.R
import ebag.hd.bean.BookCategoryBean
import ebag.hd.bean.BookCategoryListBean
    ...
    inner class MyAdapter: BaseQuickAdapter<BookCategoryListBean.TocBean, BaseViewHolder>(R.layout.item_book_category){
        override fun convert(helper: BaseViewHolder, item: BookCategoryListBean.TocBean) {
            ...
            if (item.type == "2"){ //一级目录
                ...
                textView.setTextSize(TypedValue.COMPLEX_UNIT_PX, mContext.resources.getDimension(R.dimen.x26))
            }else{//二级目录
                textView.setPadding(mContext.resources.getDimensionPixelSize(R.dimen.x20),0,0,0)
                textView.setTextSize(TypedValue.COMPLEX_UNIT_PX, mContext.resources.getDimension(R.dimen.x22))
            }
        }
    }
}